page 50101 "Vehicle Register"
{
    ...
    // Link to the Card page for detailed view/edit
    ...
    actions
    {
        area(Processing)
        {
            action("Import Vehicle Register as CSV")
            {
                Caption = 'Import Vehicle Register as CSV';
                Image = Import;
                Promoted = true;
                PromotedCategory = Process;
                ToolTip = 'Imports vehicle register data from a CSV file.';
                trigger OnAction()
                begin
                    Xmlport.Run(Xmlport::"Vehicle Register", false, true);
                end;
            }
        }
    }

    //------Page Triggers - OnOpenPage and OnClosePage-----
    // trigger OnOpenPage()
    // begin
    //     Message(
